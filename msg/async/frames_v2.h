...
struct epilogue_secure_block_t {
  uint8_t late_flags;
  uint8_t padding[CRYPTO_BLOCK_SIZE - sizeof(late_flags)];

  uint8_t ciphers_private_data[];
} __attribute__((packed));
...
private:
  ...
  uint8_t calc_num_segments(
    const std::array<segment_t, MAX_NUM_SEGMENTS>& segments)
  {
    for (uint8_t num = SegmentsNumV; num > 0; num--) {
  ...
  void fill_preamble() {
    ...
    main_preamble.tag = static_cast<uint8_t>(T::tag);
    ...
    if constexpr(SegmentsNumV > 1) {
      for (uint8_t idx = 1; idx < SegmentsNumV; idx++) {
...
struct MessageFrame : public Frame<MessageFrame,
  ...
  static MessageFrame Decode(rx_segments_t &&recv_segments) {
    ...
    for (uint8_t idx = 0; idx < std::size(recv_segments); idx++) {
