...
std::vector<std::int32_t> strict_strtovector(const std::string& s, const std::string& delim)
{
    ...
    while (pos < len)
    {
        ...
        if (find_pos < 0)
        {
            elems.push_back(std::stoi(s.substr(pos, len - pos)));
            break;
        }
        elems.push_back(std::stoi(s.substr(pos, find_pos - pos)));
