...
static const int32_t ceph_to_hostos_conv[256] = {
...
// Use the fact that the arry is initialised per default on all 0's
// And we do not translate for 0's, but return the original value.
static const int32_t hostos_to_ceph_conv[256] = {
...
int32_t ceph_to_hostos_errno(int32_t r)
{
  int sign = (r < 0 ? -1 : 1);
  int err = std::abs(r);
...
int32_t hostos_to_ceph_errno(int32_t r)
{
  int sign = (r < 0 ? -1 : 1);
  int err = std::abs(r);
